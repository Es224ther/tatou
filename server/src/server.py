...
from watermarking.xmp_create import write_xmp
from watermarking.xmp_read import read_xmp
...
# --- Routes --- 
    ...
    def create_watermark(document_id: int | None = None):
        ...
        if method != "xmp" and (not isinstance(secret, str) or not isinstance(key, str)):
        ...
        # --- XMP branch (write metadata into PDF, then insert into DB and return) ---
        if method == "xmp":
            # Construct output filename (keep consistent with existing naming convention)
            ...
            try:
                # Write XMP watermark; message is optional and fetched from request payload
                rec = write_xmp(
                    input_pdf=str(file_path),
                    output_pdf=str(dest_path),
                    issuer=os.environ.get("GROUP_NAME", "Group 8"),
                    recipient=intended_for,
                    message=(payload.get("message") or None)
                )
            except Exception as e:
                return jsonify({"error": f"XMP watermarking failed: {e}"}), 500

            # Generate link (same as current scheme: sha1(filename))
            ...
            try:
                with get_engine().begin() as conn:
                    conn.execute(
                        ...
                        """),
                        {
                            ...
                            # For XMP, we use versionId as the traceable secret stored in the DB
                            "secret": rec.get("versionId", ""),
                            "method": "xmp",
                            "position": "",
                            "path": str(dest_path)
                        },
                ...
                try:
                    # Attempt to clean up generated file if DB insertion fails
    ...
    def read_watermark(document_id: int | None = None):
        ...
        if method != "xmp" and not isinstance(key, str):
        ...
        if not file_path.exists():
            ...
            return jsonify({"error": "file missing on disk"}), 410
        # --- XMP branch (read & verify metadata signature) ---
        if method == "xmp":
            try:
                res = read_xmp(str(file_path))
            except Exception as e:
                return jsonify({"error": f"XMP read failed: {e}"}), 400

            if not res.get("ok"):
                return jsonify({
                    "error": res.get("error", "xmp-read-failed"),
                    "detail": res.get("record")
                }), 400

            # Reuse your existing return format; return versionId as the secret
            return jsonify({
                "documentid": doc_id,
                "secret": res["record"].get("versionId"),
                "method": "xmp",
                "position": ""
            }), 201
        # --- end XMP branch ---
